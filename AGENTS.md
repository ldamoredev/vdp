...
- `inbox`: D5a "Bandeja" — a frictionless universal capture queue. `InboxItem` owns raw `text` + optional `note`, `status` (`pending|triaged|discarded`), and `routedTo`/`triagedAt` (reserved for D5b triage, unused in D5a). CQBus Capture/List/Get/Discard; thin HTTP under `/api/v1/inbox`; owner-scoped. By design the inbox never writes into other modules — triage (D5b) will route via deep-link/prefill to existing create surfaces, so the inbox stays decoupled.
...
- `inbox`: universal capture items (`inbox_items`) with raw text, optional note, status, and reserved `routed_to`/`triaged_at` triage columns. No cross-module reads or writes.
