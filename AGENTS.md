...
- `projects`: active direction + project operations layer — projects own `kind` (`work|personal`), outcome, next action, focus, optional catalog `clientId` plus legacy client text, optional hourly rate/currency, and lifecycle; clients are a Projects-owned catalog; time entries log minutes by project with optional task linkage and hours reporting by client/project/week, including expected income grouped by currency and a Wallet income deep link. Archiving is reversible (`archive`/`unarchive` commands; archived projects drop out of the active list/board but stay readable in the history view with their logged hours) and logging new time against an archived project is rejected (422). Tasks remain the only work-item store and carry optional `projectId` + board column; task create/detail editing can assign a project, defaulting the board column to `backlog`. Its domain agent is intentionally read-only: it analyzes direction, board, time entries, and currency-separated hours reports; task creation and project breakdown remain Tasks-agent capabilities.
...
Agents are registered in each module runtime through `AgentRegistry`. Tasks, Wallet, Health, and Projects register agents. The Health agent covers habits/counters/goals only — never the medical section; the Projects agent is read-only and must route task creation/breakdown to Tasks. Auth, Objectives, and Inbox do not register agents.
