...
/**
 * Read model for counters. Milestone detection happens here — counters
 * need no daily interaction, so the overview load is the only reliable
 * moment to notice a crossed milestone without a scheduler. The dedupe
 * is persisted BEFORE emitting, so a failed emit costs an insight, never
 * a duplicate.
 */
export class GetCountersOverview {
    constructor(
        private readonly counters: CounterRepository,
        private readonly eventBus: EventBus,
    ) {}

    async execute(userId: string): Promise<CountersOverview> {
