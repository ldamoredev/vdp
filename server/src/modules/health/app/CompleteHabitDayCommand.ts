...
export class CompleteHabitDayCommandHandler implements RequestHandler<CompleteHabitDayCommand, { logged: boolean }> {
    ...
    async handle(command: CompleteHabitDayCommand, identity: Identity): Promise<{ logged: boolean }> {
        ...
        const targetDate = command.date ?? todayISO();
        ...
        const previousDates = await this.habits.getCompletionDates(userId, command.habitId);
        const logged = await this.habits.logCompletion(userId, command.habitId, targetDate);
    ...
    private emitSignals(userId: string, habit: Habit, previousDates: string[], date: string): void {
    ...
    private emitWeeklySignals(userId: string, habit: Habit, previousDates: string[], date: string): void {
