...
import { X, Send, Bot, Wrench, Loader2, ListChecks } from "lucide-react";
    ...
    <div className="w-96 border-l border-[var(--glass-border)] bg-[var(--sidebar)] backdrop-blur-xl flex flex-col h-full animate-slide-in-right">
      ...
      <div className="flex items-center justify-between p-4 border-b border-[var(--glass-border)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="font-medium text-sm">Asistente de Tareas</span>
        ...
        {messages.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-xs text-[var(--muted)] max-w-[250px] mx-auto leading-relaxed">
              Puedo crear tareas, revisar tu dia, pasar pendientes a manana y mas
            </p>
          </div>
        )}
        {messages.map((msg) => (
          <div key={msg.id}>
            {msg.role === "user" && (
              <div className="flex justify-end">
                <div className="bg-violet-600 text-white rounded-2xl rounded-br-md px-4 py-2.5 text-sm max-w-[80%] shadow-lg shadow-violet-500/10">
                  {msg.content}
                </div>
              </div>
            )}
      ...
      >
        <div className="flex gap-2">
          <input
            ...
            placeholder="Agregar tarea, revisar el dia..."
          ...
          <button
            ...
            className="p-2.5 bg-gradient-to-r from-violet-500 to-violet-600 text-white rounded-xl disabled:opacity-30 hover:shadow-lg hover:shadow-violet-500/20 transition-all cursor-pointer"
          >
            <Send size={16} />
          </button>
        </div>
