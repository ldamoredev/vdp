...
export default function MedicationsPage() {
  ...
  const { data: logs } = useQuery({
    ...
    queryFn: () => api.getMedicationLogs(selectedMed!),
    enabled: !!selectedMed,
  });
  ...
  const logTakenMutation = useMutation({
    mutationFn: ({ id }: { id: string }) => api.logMedication(id, { skipped: false }),
    onSuccess: () => {
  ...
  const logSkippedMutation = useMutation({
    mutationFn: ({ id }: { id: string }) => api.logMedication(id, { skipped: true }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["health"] });
    },
  });
