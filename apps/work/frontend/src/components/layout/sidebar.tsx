...
import { LayoutDashboard, FolderKanban, ListChecks, Clock, StickyNote, ChevronLeft } from "lucide-react";
    ...
    <aside className="w-60 flex flex-col bg-[var(--sidebar)] border-r border-[var(--sidebar-border)] backdrop-blur-xl">
      <div className="p-5 border-b border-[var(--sidebar-border)]">
        ...
        <div className="flex items-center gap-3 mt-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-[var(--muted)] font-medium uppercase tracking-widest">Proyectos</p>
          </div>
        </div>
      </div>
      <nav className="flex-1 p-3 space-y-1">
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all cursor-pointer ${isActive ? "bg-[var(--accent)] text-white shadow-lg shadow-amber-500/20" : "text-[var(--muted)] hover:text-[var(--foreground)] hover:bg-white/[0.04]"}`}>
